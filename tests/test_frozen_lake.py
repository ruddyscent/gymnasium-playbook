...
class LearningTests(unittest.TestCase):
    def test_nonterminal_target_includes_discounted_future_value(self):
        self.assertAlmostEqual(q_learning_target(1.0, 4.0, False, 0.9), 4.6)

    def test_terminal_target_ignores_even_a_large_next_value(self):
        self.assertEqual(q_learning_target(1.0, 100.0, True, 0.9), 1.0)

    def test_update_interpolates_one_entry(self):
        table = np.zeros((16, 4))
        table[0, 2] = 2.0
        table[1, 0] = 4.0
        expected = table.copy()
    ...
    def test_real_time_limit_still_bootstraps(self):
    ...
    def test_goal_on_time_limit_does_not_bootstrap(self):
    ...
    def test_hole_terminates_with_zero_target(self):
    ...
    def test_exploitation_breaks_ties_without_choosing_worse_actions(self):
        rng = np.random.default_rng(42)
        actions = {epsilon_greedy(np.array([0.0, 2.0, 2.0, 0.0]), 0.0, rng)
                   for _ in range(100)}
        self.assertEqual(actions, {1, 2})

    def test_full_exploration_can_choose_every_action(self):
        rng = np.random.default_rng(42)
        actions = {epsilon_greedy(np.array([10.0, 0.0, 0.0, 0.0]), 1.0, rng)
                   for _ in range(100)}
        self.assertEqual(actions, {0, 1, 2, 3})

    def test_training_is_reproducible_and_honors_time_limit(self):
    ...
    def test_training_reproduces_learned_values_not_just_initial_zeros(self):
    ...
    def test_evaluation_is_read_only_and_reports_success_and_length(self):
    ...
    def test_zero_table_times_out_with_fixed_greedy_tie_rule(self):
    ...
    def test_random_evaluation_is_reproducible(self):
        self.assertEqual(evaluate(None, 50, 7), evaluate(None, 50, 7))

    def test_invalid_inputs_fail_before_training_or_evaluation(self):
        for options in [{"episodes": 0}, {"learning_rate": 0}, {"discount": float("nan")},
